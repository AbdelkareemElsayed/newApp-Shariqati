        ...
        <div class="x_panel">
          <div class="x_title">
            ...
            <ul class="nav navbar-right panel_toolbox">
              <li><a href="{{ aurl('Courses/Videos/create') }}" class="btn btn-primary"><i class="fa fa-plus"></i>
                  {{ __('admin.addVideo') }}</a>
              ...
              <table class="table table-striped jambo_table bulk_action">
                <thead>
                  <tr class="headings">
                    ...
                    <th class="column-title">{{ __('admin.title_ar') }}</th>
                    <th class="column-title">{{ __('admin.title_en') }}</th>
                ...
                <tbody>
                  @foreach ($data as $key => $value)
                    ...
                    <tr class="{{ $pointer }}">
                      <td class=" ">{{ ++$key }}</td>
                      <td class=" ">{{ $value->title_ar }}</td>
                      <td class=" ">{{ $value->title_en }}</td>
                      <td class=" "><video src="{{ $value->video }}"></video></td>
                      <td class=" last">
                        <a data-toggle="modal" data-target=".bs-example-modal-lg{{ $value->id }}"
                          class="btn btn-danger">{{ trans('admin.delete') }}</a>
                        <a href="{{ aurl('Courses/Videos/' . $value->id . '/edit') }}"
                    ...
                    <div class="modal fade bs-example-modal-lg{{ $value->id }}" tabindex="-1" role="dialog"
                      ...
                      <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                          ...
                          <form action="{{ aurl('Courses/Videos/' . $value->id) }}" method="post">
                            @csrf
                            @method('delete')
