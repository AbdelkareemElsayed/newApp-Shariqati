        ...
        <div class="x_panel">
          <div class="x_title">
            ...
            <ul class="nav navbar-right panel_toolbox">
              <li><a href="{{ aurl('Events/create') }}" class="btn btn-primary"><i class="fa fa-plus"></i>
                  {{ __('admin.addEvent') }}</a>
              </li>
            </ul>
              ...
              <table class="table table-striped jambo_table bulk_action">
                <thead>
                  <tr class="headings">
                    ...
                    @foreach (languages() as $language)
                      <th class="column-title">{{ __('admin.name_' . $language) }}</th>
                      <th class="column-title">{{ __('admin.detail_' . $language) }}</th>
                    @endforeach
                    <th class="column-title">{{ __('admin.keywords') }}</th>
                ...
                <tbody>
                  @foreach ($data as $key => $value)
                    ...
                    <tr class="{{ $pointer }}">
                      <td class=" ">{{ ++$key }}</td>
                      @foreach ($value->eventDetails as $k => $content)
                        <td class=" ">{!! $content->name !!} </td>
                        <td class=" ">{!! $content->details !!} </td>
                      @endforeach
                      <td class=" ">
                        <ul>
                          @foreach ($value->keywords as $keyword)
                            <li>{{ json_decode($keyword)->keyword }}</li>
                          @endforeach
                        </ul>
                      </td>
                      ...
                      <td class=" last">
                        ...
                        <a href="{{ aurl('Events/' . $value->slug . '/edit') }}"
                    ...
                    <div class="modal fade bs-example-modal-lg{{ $value->id }}" tabindex="-1" role="dialog"
                      ...
                      <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                          ...
                          <form action="{{ aurl('Events/' . $value->id) }}" method="post">
                            @csrf
                            @method('delete')
