        ...
        <div class="x_panel">
          <div class="x_title">
            ...
            <ul class="nav navbar-right panel_toolbox">
              <li><a href="{{ aurl('Products/Attributes/' . $attribute_id . '/Options/create') }}"
                  class="btn btn-primary"><i class="fa fa-plus"></i>
                  {{ __('admin.addAttributeOption') }}</a>
              </li>
            </ul>
            <div class="clearfix"></div>
          </div>
                ...
                <thead>
                  <tr class="headings">
                    ...
                    @foreach (languages() as $language)
                      <th class="column-title">{{ __('admin.attribute_option_name_' . $language) }}</th>
                    @endforeach
                ...
                <tbody>
                  @foreach ($data as $key => $value)
                    ...
                    <tr class="{{ $pointer }}">
                      ...
                      @foreach (languages() as $language)
                        <td class=" ">
                          {{ $value->value->firstWhere('language', $language)->name }}
                        </td>
                      @endforeach
                      ...
                      <td class=" last">
                        ...
                        <a href="{{ aurl('Products/Attributes/' . session('attribute_id') . '/Options/' . $value->id . '/edit') }}"
                    ...
                    <div class="modal fade bs-example-modal-lg{{ $value->id }}"
                      ...
                      <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                          ...
                          <form
                            action="{{ aurl('Products/Attributes/' . session('attribute_id') . '/Options/' . $value->id) }}"
                            method="post">
                            @csrf
                            @method('delete')
