      ...
      e.rails = n = {
        ...
        blankInputs: function(t, n, r) {
          ...
          a.each(function() {
            ...
            if (!o === !r) {
              if (s.is("input[type=radio]") && a.filter('input[type=radio]:checked[name="' + s.attr("name") +
                  '"]').length) {
                return true
              }
              i = i.add(s)
...
@include('dashboard.layouts.sidNav')
<!-- top navigation -->
<div class="top_nav">
  <div class="nav_menu">
      ...
      <ul class="navbar-right">
        ...
        <li role="presentation" class="nav-item dropdown open">
          ...
          <ul class="dropdown-menu list-unstyled msg_list" style="height: 231px; overflow: auto;" role="menu"
            ...
            @foreach (DB::table('notifications')->orderBy('id', 'desc')->where('user_id', auth('admin')->user()->id)->get()
    as $notification)
              <li class="nav-item">
    ...
    <form class="form-add-locale autotranslate-block-group hidden" method="POST" role="form"
      ...
      <div class="row">
        <div class="col-sm-6">
          ...
          <p>
            <code>If you would like to use Google Translate API, install tanmuhittin/laravel-google-translate and enter
              your Google Translate API key to config file laravel_google_translate</code>
          </p>
          <?php endif; ?>
          <div class="form-group">
            <input type="hidden" name="with-translations" value="1">
            <input type="hidden" name="file" value="<?= $group ?>">
            <button type="submit" class="btn btn-default btn-block" data-disable-with="Adding..">Auto translate missing
              translations</button>
          </div>
        </div>
      </div>
        ...
        <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false); ?>">
          ...
          <td>
            <a href="#edit" class="editable status-<?php echo $t ? $t->status : 0; ?> locale-<?php echo $locale; ?>"
              data-locale="<?php echo $locale; ?>" data-name="<?php echo $locale . '|' . htmlentities($key, ENT_QUOTES, 'UTF-8', false); ?>" id="username" data-type="textarea"
              data-pk="<?php echo $t ? $t->id : 0; ?>" data-url="<?php echo $editUrl; ?>"
